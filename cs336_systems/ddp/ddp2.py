...
def ddp_train_worker(rank: int, world_size: int, cfg: TrainConfig, args: argparse.Namespace):
    ...
    if rank == 0 and args.use_wandb:
        wandb_run = init_wandb(
            ...
            watch_model=False,   # 你要看梯度图可以改 True
            log_config=True,
        )

        # 保存 loss 曲线历史，用于 log_validation_metrics 画图
        train_losses_hist: list[float] = []
        val_losses_hist: list[float] = []
        train_iters_hist: list[int] = []
        val_iters_hist: list[int] = []
    else:
        train_losses_hist = []
        val_losses_hist = []
        train_iters_hist = []
        ...
        if need_log:
            ...
            if rank == 0:
                ...
                if wandb_run is not None:
                    # 1. 追加到历史，用于画曲线
                    train_losses_hist.append(loss.item())
                    val_losses_hist.append(val_loss)
                    train_iters_hist.append(global_step)
                    val_iters_hist.append(global_step)
                    ...
                    log_training_metrics(
                        ...
                        loss=loss.item(),
                        learning_rate=lr,
                        elapsed_time=None,          # 用内部 start_time 算
                        iter_time_s=avg_iter_time,  # 自定义额外信息
                    )
                    ...
                    log_validation_metrics(
                        ...
                        train_iterations=train_iters_hist,
                        val_iterations=val_iters_hist,
                    )
